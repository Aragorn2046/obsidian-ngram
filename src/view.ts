...
export class BlueprintView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf, plugin: NgramPlugin) {
    super(leaf);
    this.plugin = plugin;
  }
  ...
  async onOpen(): Promise<void> {
    ...
    contentEl.empty();
    contentEl.addClass("ngram-container");
    ...
    try {
      ...
      console.error("[Ngram] Failed to initialize:", error);
      this.showError(
  ...
  private async doRescan(): Promise<void> {
    try {
      ...
      console.error("[Ngram] Rescan failed:", error);
    }
  }
